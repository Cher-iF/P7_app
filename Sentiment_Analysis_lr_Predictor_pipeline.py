...
if st.button('Analyze'):
    with st.spinner('Analyzing the text …'):
        ...
        if pred == 0:
            ...
            #st.write('Results with Pipeline from hugginFace')
            #st.success(pred_pipe)
            ...
            #st.write('Results with Pipeline from hugginFace')
            #st.success(pred_pipe )
st.header('want to see huggin pipe result  :')
if st.button('pipe it '):
    with st.spinner('Analyzing the text …'):
         st.success(pred_pipe)
